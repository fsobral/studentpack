...
subroutine generate_x(x, n, W, H, A)
	...
	real(kind=8) :: Tz(A,2), Ts(A,2), Td(A,2), Pdist_t(A,6), ajuste_t(A,6), provaL, P, L
	real(kind=8) :: dist_t(1,6), dist, aux, Z(A,2), S(A,2), D(A,2), camada, camadadupla
	...
	do i=2,A
		S(i,1) = CEILING(real((A - CEILING(real(i)/2.0))/ real(i))) + 1.0
		S(i, 2) = i	
	end do
